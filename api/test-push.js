...
export default async function handler(req, res) {
  ...
  const envCheck = {
    ...
    vapidPubPreview: process.env.VAPID_PUBLIC_KEY?.slice(0, 20),
  };
  if (!userId) return res.status(200).json({ envCheck });
  try {
    ...
    res.status(200).json({
      ...
      endpointPreview: data?.[0]?.subscription?.endpoint?.slice(0, 60),
    });
  } catch (err) {
