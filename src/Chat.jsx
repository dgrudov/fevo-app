...
export default function Chat({ event, user, myName, onBack }) {
 ...
 const sendMessage = async () => {
  ...
  return (
<div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column", background: "#f8f5f0", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 100 }}>      
      {/* Header */}
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id} style={{ display: "flex", flexDirection: "column", alignItems: isMe ? "flex-end" : "flex-start" }}>
              {showName && (
                <div style={{ fontSize: 12, color: "#8a7a6a", marginBottom: 4, marginLeft: 4 }}>{msg.user_name}</div>
              )}
              <div style={{ display: "flex", alignItems: "flex-end", gap: 8, flexDirection: isMe ? "row-reverse" : "row" }}>
                {!isMe && (i === messages.length - 1 || messages[i+1]?.user_id !== msg.user_id) && (
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: event.color + "55", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "#a89f92", marginTop: 2, marginLeft: isMe ? 0 : 36, marginRight: isMe ? 4 : 0 }}>
      ...
      <div style={{ padding: "12px 20px 32px", background: "#fff", borderTop: "1px solid #e8e3db", display: "flex", gap: 10, alignItems: "flex-end", flexShrink: 0 }}>
        ...
        <button onClick={sendMessage} disabled={!newMessage.trim()} style={{
          width: 44, height: 44, borderRadius: "50%", border: "none", cursor: newMessage.trim() ? "pointer" : "not-allowed",
